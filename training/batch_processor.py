...
class BatchProcessor:
    ...
    def __init__(self, config, model, tokenizer, agrpo_algo, reward_extractor, accelerator):
        self.config = config
        self.model = model
        # self.ref_model = ref_model ### <--- THIS LINE HAS BEEN REMOVED
        self.tokenizer = tokenizer
        self.agrpo_algo = agrpo_algo
        self.reward_extractor = reward_extractor
        self.accelerator = accelerator
            ...
            for i in range(num_examples):
                ...
                for j in range(self.config.num_responses_per_query):
                    ...
                    with torch.no_grad():
                        ...
                        # Use the main model with LoRA adapters temporarily disabled
                        # This replaces the need for a separate ref_model
    ...
    def _compute_metrics(self, responses_data: Dict, loss_data: Dict) -> Dict[str, Any]:
        """Compute batch metrics"""
        all_rewards_flat = [r for rewards in responses_data['rewards'] for r in rewards]
